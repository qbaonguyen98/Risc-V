...
STORE:
		...
		slti t6, t0, 9
		bnez t6, NETX_2ND
		sw t2, NEW_ADDR1(zero)
		sw s2, ST_ADDR1(zero)
		j EXIT_STORAGE
NETX_2ND:
		slti t6, t0, 8
		bnez t6, NEXT_3RD
		sw t2, NEW_ADDR2(zero)
		sw s2, ST_ADDR2(zero)
		j EXIT_STORAGE
NEXT_3RD:
		slti t6, t0, 7
		bnez t6, NEXT_4TH
		sw t2, NEW_ADDR3(zero)
		sw s2, ST_ADDR3(zero)
		j EXIT_STORAGE
NEXT_4TH:
		slti t6, t0, 6
		bnez t6, NEXT_5TH
		sw t2, NEW_ADDR4(zero)
		sw s2, ST_ADDR4(zero)
		j EXIT_STORAGE
NEXT_5TH:
		slti t6, t0, 5
		bnez t6, NEXT_6TH
		sw t2, NEW_ADDR5(zero)
		sw s2, ST_ADDR5(zero)
		j EXIT_STORAGE
NEXT_6TH:
		slti t6, t0, 4
		bnez t6, NEXT_7TH
		sw t2, NEW_ADDR6(zero)
		sw s2, ST_ADDR6(zero)
		j EXIT_STORAGE
NEXT_7TH:
		slti t6, t0, 3
		bnez t6, NEXT_8TH
		sw t2, NEW_ADDR7(zero)
		sw s2, ST_ADDR7(zero)
		j EXIT_STORAGE
NEXT_8TH:
		slti t6, t0, 2
		bnez t6, NEXT_9TH
		sw t2, NEW_ADDR8(zero)
		sw s2, ST_ADDR8(zero)
		j EXIT_STORAGE
NEXT_9TH:
		slti t6, t0, 1
		bnez t6, NEXT_10TH
		sw t2, NEW_ADDR9(zero)
		sw s2, ST_ADDR9(zero)
		j EXIT_STORAGE
NEXT_10TH:
		sw t2, NEW_ADDR10(zero)
		sw s2, ST_ADDR10(zero)
EXIT_STORAGE:
		beqz t0, ENDTASK
		addi t0, t0, -1
		li s5, 1
